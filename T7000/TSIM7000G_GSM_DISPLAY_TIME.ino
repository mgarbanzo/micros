...
void loop(){
  ...
  * * */
  ...
  // LOOKING FOR TIME  
  if (modem.isGprsConnected()) {
    Serial.println("GPRS connected. Preparing to fetch data...");

    // Specify server and resource for World Time API
    const char server[] = "worldtimeapi.org"; 
    const char resource[] = "/api/timezone/America/Costa_Rica";
    const int port = 80;
    ...
    while (client.connected() && millis() - timeout < 10000L) {
      // Print available data from server
